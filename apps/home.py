...
app.clientside_callback(
    """
    function(pass_keys, user_keys, current_clicks) {
        // Initialize only once
        if (typeof window.enterHandlerInitialized === 'undefined') {
            ...
            // Add to both fields
            document.getElementById('login_username').addEventListener('keypress', handleEnter);
            document.getElementById('login_password').addEventListener('keypress', handleEnter);
            
            window.enterHandlerInitialized = true;
        }
...
def loginprocess(loginbtn, useremail, password, currentuserid, pathname):    
    ...
    return [alert_color, alert_text, alert_open, username_class, password_class, currentuserid, accesstype, pathname]  # Returning the current URL pathname if login fails
