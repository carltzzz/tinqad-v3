    ...
    if pathname == '/acad_heads_directory':
        ...
        sql = """
            SELECT 
                ...
                clusters.cluster_shortname AS "Academic Cluster",
            ...
            FROM
                ...
                LEFT JOIN public.deg_unit ON unithead_deg_unit_id = deg_unit.deg_unit_id
                WHERE
        ...
        cols = [
            'ID', 'Academic Cluster', 'Main Unit', 'Sub-unit', 'Full Name', 'Up Mail',
        ...
        if selected_years:
            year_int = int(selected_years)
            df = df[df['Start Term'].dt.year == year_int]
        ...
        df['Start Term'] = (
            pd.to_datetime(df['Start Term'], errors='coerce')
            .dt.strftime('%Y-%m-%d')
            .fillna('')    # now true nulls are '' instead of 'NaT'
            .str.slice(0, 10)
        )
        ...
        if df.shape[0] > 0:
            ...
            df = df[['Academic Cluster', 'Main Unit', 'Sub-unit', 'Full Name', 'Up Mail',
