...
dbc.Row(
    dbc.Col(
        dbc.Card(
            [
                dbc.CardHeader(html.B("ISO Facilitators Tracker")),
                dbc.CardBody(
                    html.Div(
                        id='isofacimoredetails_list',
                        style={
                            'marginTop': '20px',
                            'overflowX': 'auto',
                            'overflowY': 'auto',
...
@app.callback(
    Output('isofacimoredetails_list', 'children'),
    [Input('url', 'pathname')]
)
def isofacimoredetails_loadlist(pathname):
    if pathname == '/dashboard/more_details':
        sql = """
            SELECT 
                c.cluster_shortname AS "Cluster",
                cl.college_shortname AS "Academic Unit",
                CONCAT(i.isofaci_fname, ' ', 
           CASE 
               WHEN i.isofaci_mname IS NOT NULL AND i.isofaci_mname <> '' 
               THEN SUBSTRING(i.isofaci_mname FROM 1 FOR 1) || '. ' 
               ELSE '' 
           END,
                i.isofaci_sname) AS "Name",
                i.isofaci_upmail AS "UP mail",
                i.isofaci_fac_posn AS "Faculty Position",
                i.isofaci_staff_posn AS "Staff Position",
                i.isofaci_rolecuqa AS "CUQA Role",
                i.isofaci_remarks AS "Remarks"
            FROM iqateam.iso_facilitators i
            LEFT JOIN public.clusters c ON i.isofaci_cluster_id = c.cluster_id
            LEFT JOIN public.college cl ON i.isofaci_college_id = cl.college_id
            WHERE i.isofaci_del_ind IS FALSE;

        """
        cols = ["Cluster", "Academic Unit", "Name", "UP mail", "Faculty Position", "Staff Position", "CUQA Role", "Remarks"]
        df = db.querydatafromdatabase(sql, [], cols)
        ...
        if not df.empty:
            ...
            df.index += 1
            df.index.name = 'No.'
            count_div = html.Div(f"Total ISO Facilitators: {len(df)}")
            df.reset_index(inplace=True)
            table = dbc.Table.from_dataframe(df, striped=True, bordered=True, hover=True, size='sm')
            return html.Div([count_div, table])
        else:
