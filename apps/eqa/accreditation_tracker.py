...
def get_available_years():
    ...
    sql = """
    SELECT DISTINCT EXTRACT(YEAR FROM arep_sched_startdate) AS year
                    ...
                    [
                        ...
                        dbc.Row(
                            [
                                dbc.Col(
                                    dbc.Button(
                                        "➕ Add New Activity", color="primary",
                                        href='/assessment_tracker/assessment_details?mode=add',
                                    ),
                                    width="auto",
                                ),
                                dbc.Col(
...
def accreditationtracker_loadlist(pathname, searchterm, eqa_types, selected_month, selected_years):
    if pathname == '/assessment_tracker': 
        ...
        if selected_month:  # Add condition to filter by selected month
            sql += " AND EXTRACT(MONTH FROM arep_sched_startdate) = %s"
            values.append(selected_month)

        if selected_years:  # selected_years will be a list of selected years
            sql += " AND EXTRACT(YEAR FROM arep_sched_startdate) IN %s"
