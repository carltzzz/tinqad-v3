                                    ...
                                    [
                                        dbc.Row(
                                            [
                                                dbc.CardBody([
                                                    dbc.Table(
            # Example static table data; replace with your dynamic data as needed
                                                    [
                                                        html.Thead(html.Tr([html.Th("Column 1"), html.Th("Column 2"), html.Th("Column 3")])),
                                                        html.Tbody(
                                                            [
...
@app.callback(
    Output('rep_sar_forchecking', 'children'),
    [Input('url', 'pathname')]
) 
def eqa_sar_forchecking(pathname):
    if pathname == '/eqa_dashboard': 
        ...
        SELECT 
            ...
            rep.rep_currentdate AS "Date Submitted"
        FROM 
            eqateam.reports rep
        JOIN 
            eqateam.program_details pd ON rep.rep_degree_programs_id = pd.programdetails_id
        WHERE 
            rep.rep_checkstatus = 'For Checking' AND rep.rep_report_type = 'SAR'
        """
