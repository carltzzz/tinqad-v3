...
# ────────────────────────────────────────────────────────────────────────────────
# 1) Dynamically fetch all SDG-2 metrics (code + description) from the database
# ────────────────────────────────────────────────────────────────────────────────
...
for _, row in metrics_df.iterrows():
    ...
    inner_div = html.Div(
        id={"type": "sdg2_list", "index": metric_code},
        style={
            "marginTop": "20px",
            "overflowX": "auto",
            "overflowY": "auto",
            ...
            title=html.Div(
                [
                    ...
                    html.Div(
                        ...
                        id={"type": "sdg2_header_alert", "index": metric_code},
                        style={"display": "none"},
                    ),
                ],
                style={
...
sdg2_accordion = html.Div([
    dcc.Store(id="sdg2_admin_toload", storage_type="memory", data=None),
    dcc.Store(id="sdg2_update_counter", storage_type="memory", data=0),
    ...
    dbc.Modal([
        dbc.ModalHeader("Evaluate Evidence", id="sdg2_admin_modal_header"),
        dbc.ModalBody([
            dbc.Row([
                dbc.Col(html.Label("Status"), width=4),
                dbc.Col(dbc.Select(id="sdg2_admin_modal_status", placeholder="Select status"), width=8),
            ], className="mb-3"),
            dbc.Row([
                dbc.Col(dbc.Textarea(
                    id="sdg2_admin_modal_comments",
        ...
        dbc.ModalFooter([
            dbc.Button("Save", id="sdg2_admin_modal_save", color="primary"),
            dbc.Button("Cancel", id="sdg2_admin_modal_cancel", color="secondary", className="ms-2"),
        ]),
    ], id="sdg2_admin_modal", is_open=False, centered=True, backdrop="static"),
])
...
@app.callback(
    Output({"type": "sdg2_list", "index": MATCH}, "children"),
    [ Input("url", "pathname"), Input("sdg2_update_counter", "data") ],
    [ State({"type": "sdg2_list", "index": MATCH}, "id") ],
)
def load_sdg2_metric_list(pathname, counter_update, id):
    if pathname != "/SDG_evidencelist/sdg2":
        raise PreventUpdate
...
@app.callback(
    [
        Output("sdg2_admin_modal",          "is_open"),
        Output("sdg2_admin_modal_status",   "options"),
        Output("sdg2_admin_modal_comments", "value"),
        Output("sdg2_admin_toload",         "data"),
        Output("sdg2_admin_modal_status",   "value"),
        Output("sdg2_admin_alert",          "is_open"),
        Output("sdg2_admin_alert",          "color"),
        Output("sdg2_admin_alert",          "children"),
        Output("sdg2_update_counter",       "data"),
    ],
    [
        Input({"type": "eval",   "index": ALL}, "n_clicks"),
        Input("sdg2_admin_modal_save",    "n_clicks"),
        Input("sdg2_admin_modal_cancel",  "n_clicks"),
    ],
    [
        State("sdg2_admin_modal",         "is_open"),
        State("sdg2_admin_toload",        "data"),
        State("sdg2_admin_modal_status",  "value"),
        State("sdg2_admin_modal_comments","value"),
        State("sdg2_update_counter",      "data"),
    ],
...
def manage_admin_modal(eval_clicks, save_clicks, cancel_clicks,
    ...
    if prop_id.startswith("sdg2_admin_modal_save") and toload:
    ...
    # --- CANCEL ---
    if prop_id.startswith("sdg2_admin_modal_cancel"):
        return (False, no_update, no_update, None, None,
                False, "", "", no_update)
...
# ────────────────────────────────────────────────────────────────────────────────
@app.callback(
    Output({"type": "sdg2_header_alert", "index": ALL}, "style"),
    Input("sdg2_update_counter", "data"),
    State({"type": "sdg2_header_alert", "index": ALL}, "id"),
)
